...
PyObject* py_decode_vertex_buffer(PyObject* self, PyObject* const* args, Py_ssize_t nargs) {
    ...
    if (!PyBytes_Check(args[0])) {
        PyErr_SetString(PyExc_TypeError, "input must be a bytes object (limited API 3.10 build)");
        return nullptr;
    }
    ...
    const char* src = PyBytes_AsString(args[0]);
    if (!src) return nullptr;
    const Py_ssize_t src_size = PyBytes_Size(args[0]);
